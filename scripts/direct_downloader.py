...
end_date=start_date
output_file = '../out/direct_downloader/IWB_holdings_20220823.csv'
# ---------------------------------------------------------------------------------------------
...
if __name__ == '__main__':
    request_url = f'{etf_landing_page}/1467271812596.ajax?' \
                  f'fileType=json&tab=all&asOfDate={end_date}'
    print(f'requesting: {request_url}')

    response = requests.get(request_url)

    if response is None:
        print(f'{"!" * 10}\nREQUEST FAILED({end_date}):\n'
        ...
        if response.status_code != 200:
            ...
            if holdings_df.shape[0] == 0:
                ...
                holdings_df.insert(loc=0,
                                   column='as_of_date',
                                   value=f'{end_date[:4]}-{end_date[4:6]}-{end_date[6:]}')  # %Y-%m-%d
    
   #TODO: Figure out CWD issue.  kjkj

    print(os.getcwd())
    holdings_df.to_csv('./out/test.csv')
